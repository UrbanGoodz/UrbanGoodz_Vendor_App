    ...
    return Obx(
      () {
        if (c.filteredPromotions.isEmpty) {
          return Center(
            child: Column(
              ...
              children: [
                Icon(Icons.local_offer, size: 64, color: AppTheme.dark.withOpacity(0.2)),
        ...
        return ListView.builder(
          ...
          itemBuilder: (_, i) => _PromotionCard(promotion: c.filteredPromotions[i], controller: c),
        );
      },
    );
  }

  void _showCreatePromotionDialog(BuildContext context, PromotionsController c) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('Create Promotion', style: TextStyle(color: AppTheme.dark)),
        content: SingleChildScrollView(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              TextField(controller: titleCtrl, decoration: const InputDecoration(labelText: 'Title', hintText: 'e.g. Summer Harvest Sale')),
              const SizedBox(height: 8),
              TextField(controller: descCtrl, decoration: const InputDecoration(labelText: 'Description', hintText: 'Describe your promotion')),
              const SizedBox(height: 8),
              TextField(controller: codeCtrl, decoration: const InputDecoration(labelText: 'Promo Code', hintText: 'e.g. SUMMER20')),
              const SizedBox(height: 8),
              TextField(controller: discountCtrl, decoration: const InputDecoration(labelText: 'Discount %', hintText: 'e.g. 20'), keyboardType: TextInputType.number),
            ],
          ),
        ),
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              if (titleCtrl.text.isNotEmpty) {
                c.createPromotion(PromotionModel(
...
class _PromotionCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final isActive = promotion.isActive && promotion.endDate.isAfter(DateTime.now());
    return Card(
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: (isActive ? AppTheme.primary : Colors.grey).withOpacity(0.15),
            ...
            Text(
              ...
              style: TextStyle(fontSize: 13, color: AppTheme.dark.withOpacity(0.7)),
            ),
            ...
            Row(
              children: [
                ...
                _buildInfoChip(Icons.date_range, '${_formatDate(promotion.startDate)} - ${_formatDate(promotion.endDate)}'),
                const SizedBox(width: 8),
                _buildInfoChip(Icons.people, '${promotion.usageCount}/${promotion.usageLimit} used'),
              ],
            ),
            const SizedBox(height: 8),
            Row(
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(text, style: TextStyle(fontSize: 11, color: AppTheme.dark.withOpacity(0.7))),
        ],
      ),
    );
  }
