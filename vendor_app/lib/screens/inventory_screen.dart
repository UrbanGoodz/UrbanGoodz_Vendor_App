...
class InventoryScreen extends StatelessWidget {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(child: _buildInventoryList(c)),
        ],
      ),
          ...
          Expanded(
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                prefixIcon: const Icon(Icons.search, color: AppTheme.dark, size: 20),
                filled: true,
  ...
  Widget _buildCategoryChips(InventoryController c) {
    final categories = ['all', 'Produce', 'Bakery', 'Meat', 'Meal Kits', 'Beverages', 'Condiments', 'Dairy', 'Pantry'];
    return Obx(
      () => SizedBox(
        height: 48,
        child: ListView(
          ...
          children: categories.map((cat) {
            final selected = c.selectedCategory.value.toLowerCase() == cat.toLowerCase();
            return Padding(
  ...
  Widget _buildWarningSection(String title, List<InventoryItemModel> items, InventoryController c) {
    return Container(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 4),
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(
        color: title == 'Out of Stock' ? Colors.red.withOpacity(0.08) : AppTheme.primary.withOpacity(0.08),
        borderRadius: BorderRadius.circular(8),
        border: Border.all(
          color: title == 'Out of Stock' ? Colors.red.withOpacity(0.3) : AppTheme.primary.withOpacity(0.3),
        ),
      ),
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(
                title == 'Out of Stock' ? Icons.error_outline : Icons.warning_amber,
                size: 16,
  ...
  Widget _buildInventoryList(InventoryController c) {
    return Obx(
      () {
        if (c.filteredItems.isEmpty) {
          return Center(
            child: Column(
              ...
              children: [
                Icon(Icons.inventory_2, size: 64, color: AppTheme.dark.withOpacity(0.2)),
        ...
        return ListView.builder(
          ...
          itemBuilder: (_, i) => _InventoryCard(item: c.filteredItems[i], controller: c),
        );
      },
    );
  }
  ...
  void _showAddItemDialog(BuildContext context, InventoryController c) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('Add Inventory Item', style: TextStyle(color: AppTheme.dark)),
        content: SingleChildScrollView(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              TextField(controller: nameCtrl, decoration: const InputDecoration(labelText: 'Name', hintText: 'e.g. Organic Avocados')),
              const SizedBox(height: 8),
              TextField(controller: skuCtrl, decoration: const InputDecoration(labelText: 'SKU', hintText: 'e.g. AVO-ORG-001')),
              const SizedBox(height: 8),
              TextField(controller: priceCtrl, decoration: const InputDecoration(labelText: 'Price', hintText: 'e.g. 5.99'), keyboardType: TextInputType.number),
              const SizedBox(height: 8),
              TextField(controller: stockCtrl, decoration: const InputDecoration(labelText: 'Stock Qty', hintText: 'e.g. 50'), keyboardType: TextInputType.number),
              const SizedBox(height: 8),
              TextField(controller: thresholdCtrl, decoration: const InputDecoration(labelText: 'Low Stock Threshold', hintText: 'e.g. 10'), keyboardType: TextInputType.number),
            ],
          ),
        ),
        actions: [
...
class _InventoryCard extends StatelessWidget {
          ...
          child: Row(
            children: [
              Container(
                ...
                child: Icon(Icons.inventory_2, color: AppTheme.primary.withOpacity(0.7)),
              ),
  ...
  void _showEditStockDialog(BuildContext context) {
    ...
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text('Update Stock - ${item.name}', style: const TextStyle(color: AppTheme.dark)),
        content: TextField(
          controller: qtyCtrl,
          keyboardType: TextInputType.number,
          decoration: const InputDecoration(labelText: 'Quantity', hintText: 'Enter new stock quantity'),
        ),
        actions: [
          TextButton(onPressed: () => Get.back(), child: const Text('Cancel')),
          ElevatedButton(
